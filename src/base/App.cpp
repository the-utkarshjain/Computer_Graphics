...
vector<Vertex> loadUserGeneratedModel() {
	...
	for (auto i = 0u; i < faces; ++i) {
		...
		v0.position = Vec3f(-radius * FW::cos(angle_increment * i), -height, radius * FW::sin(angle_increment * i));
		v1.position = Vec3f(-radius * FW::cos(angle_increment * (i+1)), -height, radius * FW::sin(angle_increment * (i+1)));
		v2.position = Vec3f(0, 0, 0);
		// Calculate the normal of the face from the positions and use it for all vertices.
		v0.normal = v1.normal = v2.normal = normalize(cross(v1.position - v0.position, v2.position - v0.position));
